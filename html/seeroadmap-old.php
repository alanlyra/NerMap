...
<!-- query: SELECT DISTINCT id_prospec, nome_prospec, assunto_prospec, ano_prospec, num_textos_prospec, status_ren_prospec, conf_prospec, usuario_prospec, palavra, ordem, tag FROM prospec p, texto t, ren r WHERE t.id_texto = p.id_prospec AND r.id_ren = t.id_texto and t.ordem = r.ordem_texto and id_prospec = 1 AND r.tag != 'O' ORDER BY ordem -->
            ...
            <ul class="timeline">
              ...
              <?php 
                      $search_results=get_data('SELECT * FROM arquivos a INNER JOIN texto t ON t.id_texto = a.id_arquivo INNER JOIN ren r ON r.id_ren = t.id_texto INNER JOIN prospec p ON p.id_prospec = a.id_prospec_arquivo AND r.ordem_texto = t.ordem WHERE id_arquivo = 38');
                      ...
                      $results_max = pg_num_rows($search_results);
                        ...
                        while($result=pg_fetch_object($search_results)) {
                          ...
                          //echo "<script>console.log('" . strval($result->palavra) . "');</script>";

                          //$palavra = str_replace('\'', '', strval($result->palavra));
                          ...
                          if($palavra == ".") {
                            ...
                            echo "<script>console.log('" . $array_sections[$i_section][info] . "');</script>";
                         ...
                         //TODO for com os elementos do array que sao is_prospec = true
                          if($section[is_prospec]) {
                            if($side_left)
                            echo "<li>";
                            else
                            ...
                            echo "<div class='tl-circ'></div>
                                    <div class='timeline-panel'>
                                      <div class='tl-heading'>
                                        ...
                                        <p><small class='text-muted'><i class='glyphicon glyphicon-time'></i>".$section[date]."</small></p>
                                      </div>
                                      <div class='tl-body'>
                                        <p>".$section[info]."</p>
                                      </div>
