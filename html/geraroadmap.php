...
require_once 'system.php';
  ...
  if (isset($argc)) {
    for ($i = 0; $i < $argc; $i++) {
      ...
      if(has_arquivo_processando($id_prospec))
        ...
        $update_on_prospec = set_data("UPDATE prospec SET status_ren_prospec = 'ERRO' where id_prospec = $1", array($id_prospec));
      else
  ...
  function has_arquivo_com_erro($id_prospec) {   
    $number1 = get_data("SELECT COUNT(*) FROM arquivos WHERE id_prospec_arquivo = ".$id_prospec." AND status_ren = 'ERRO'");
    $row = pg_fetch_array($number1);        
    if($row[0] == 0)
      return false;
    else
